...
bool SendCoinsDialog::handleURI(const QString &uri)
{
    ...
    {
        CMarteXAddress address(rv.address.toStdString());
        if (!address.IsValid())
            return false;
        pasteEntry(rv);
        return true;
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::NarrationTooLong:
        ...
        break;
    case WalletModel::MartexFee:
        msgParams.first = tr("A fee %1 times higher than %2 per recipient is considered an martexly high fee.").arg(10000).arg(BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), MIN_RELAY_TX_FEE));
        break;
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CMarteXAddress addr = CMarteXAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid MarteX address"));
        }
        else // Valid address
        {
            CPubKey pubkey;
